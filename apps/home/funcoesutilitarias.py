...
def criar_projeto(dados_projeto, id_usuario):
    ...
    try:
        ...
        try: 
            ...
            projeto_usuario = ProjetosUsuários(user=usuario, projeto=projeto)
...
def filtrar_projetos_usuario(usuario):
    projetos = ProjetosUsuários.objects.filter(user=usuario.id)
    return projetos

def formatar_projetos_usuario(projetos):
    retorno = []
